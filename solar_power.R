# fos <- list.files("data/", recursive=FALSE)
# 
# spower <- data.frame()
# for (fo in fos[1:5]){
#   st <- Sys.time()
#   fs <- list.files(sprintf("data/%s/", fo))
#   fs <- fs[grep("HA4",fs,invert=TRUE)]
#   
#   spower_state <- do.call(rbind, sapply(fs, function(f){
#     f_df <- read.csv(sprintf("data/%s/%s", fo, f))
#     f_df$LocalTime <- as.POSIXlt(f_df$LocalTime, format="%m/%d/%y %H:%M")
#     
#     
#     f_df$State <- fo
#     
#     f_meta <- unlist(str_split(f, "_"))
#     f_df$DataType <- f_meta[1]
#     f_df$Lat <- as.numeric(f_meta[2])
#     f_df$Lon <- as.numeric(f_meta[3])
#     f_df$PVType <- f_meta[5]
#     f_df$CapacityMW <- f_meta[6]
#     f_df$TimeInterval <- as.numeric(f_meta[7])
#     
#     return(f_df)
...
#     spower_state %>%
#       filter(PVType == "DPV") %>%
#       group_by(DataType, State, CapacityMW, LocalTime) %>%
#       summarise(Power = sum(Power.MW.))
#   )
#   
#   print(sprintf("%s finished in %.2f s", fo, as.numeric(Sys.time()- st, "secs")))
# }

# spower %>%
#   # filter(PVType == "DPV") %>%
#   group_by(DataType, CapacityMW, LocalTime) %>%
#   summarise(Power = sum(Power.MW.)) -> spower_sum

# spower_sum_da <- spower_sum %>%
#   pivot_wider(names_from=DataType, values_from=Power) %>%
#   drop_na() %>%
#   filter(month(LocalTime) <= 1) %>%
#   group_by(CapacityMW) %>%
#   mutate(train = LocalTime != max(LocalTime)) %>% 
#   group_by(CapacityMW, train) %>%
#   summarise(da_err = sqrt(mean( (Actual-DA)^2)))
